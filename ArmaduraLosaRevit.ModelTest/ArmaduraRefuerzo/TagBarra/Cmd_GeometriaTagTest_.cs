...
namespace ArmaduraLosaRevit.Model.ElementoBarraRoom.Tag
{
...

    {
        ...
        public Result Execute(ExternalCommandData commandData,ref string messages, ElementSet elements)
        {
            ...
            app.ActiveUIDocument.ActiveView = (View)_doc.GetElement(new ElementId(1164163));
            ...
            SolicitudBarraDTO _solicitudBarraDTO = new SolicitudBarraDTO(app, "F4", UbicacionLosa.Derecha, TipoConfiguracionBarra.refuerzoInferior, false);
            ...
            Debug.Assert(_geometriaTag.listaTag[0].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_A_50_0");
            Debug.Assert(_geometriaTag.listaTag[0].posicion.DistanceTo(new XYZ(-52.034120735, 197.151619160, 26.541994751))<0.1);

            Debug.Assert(_geometriaTag.listaTag[1].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_B_50_0");
            Debug.Assert(_geometriaTag.listaTag[1].posicion.DistanceTo(new XYZ(-54.166666667, 196.561067979, 26.541994751)) < 0.1);
            
            Debug.Assert(_geometriaTag.listaTag[2].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_C_50_0");
            Debug.Assert(_geometriaTag.listaTag[2].posicion.DistanceTo(new XYZ(-47.882033690, 196.265792388, 26.541994751)) < 0.1);

            Debug.Assert(_geometriaTag.listaTag[3].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_F_50_0");
            Debug.Assert(_geometriaTag.listaTag[3].posicion.DistanceTo(new XYZ(-48.046075685, 196.790726772, 26.541994751)) < 0.1);

            Debug.Assert(_geometriaTag.listaTag[4].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_L_50_0");
            Debug.Assert(_geometriaTag.listaTag[4].posicion.DistanceTo(new XYZ(-44.765235789, 196.823535171, 26.541994751)) < 0.1);

            Debug.Assert(_geometriaTag.listaTag[5].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_D_50_0");
            Debug.Assert(_geometriaTag.listaTag[5].posicion.DistanceTo(new XYZ(-36.663385827, 196.561067979, 26.541994751)) < 0.1);

            Debug.Assert(_geometriaTag.listaTag[6].nombreFamilia == "M_Path Reinforcement Tag(ID_cuantia_largo)_E_50_0");
