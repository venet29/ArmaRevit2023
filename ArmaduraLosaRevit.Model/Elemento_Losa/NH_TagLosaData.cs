...
namespace ArmaduraLosaRevit.Model.Elemento_Losa
{
    ...
    public class NH_TagLosaData
    {
        ...
        /// Selected beams
        /// </summary>
        private List<FamilyInstance> m_LosaList = new List<FamilyInstance>();

        /// <summary>
        /// Tag types whose category is "Structural Framing Tags"
        /// </summary>
        private List<FamilySymbolWrapper> m_categoryTagTypes = new List<FamilySymbolWrapper>();

        /// <summary>
        /// Tag types whose category is "Multi-Category Tags"
        /// </summary>
        private List<FamilySymbolWrapper> m_multiCategoryTagTypes = new List<FamilySymbolWrapper>();

        /// <summary>
        /// Tag types whose category is "Material Tags"
        /// </summary>
        private List<FamilySymbolWrapper> m_materialTagTypes = new List<FamilySymbolWrapper>();
        ...
        public NH_TagLosaData(ExternalCommandData commandData)
        {
            //Get beams selected 
            ...
            while (itor.MoveNext())
            {
                ...
                {
                    m_LosaList.Add(familyInstance);
                }
            }
            if (m_LosaList.Count < 1)
            {
        ...
        public void CreateTag(TagMode tagMode, FamilySymbolWrapper tagSymbol, bool leader, TagOrientation tagOrientation)
        {
            foreach (FamilyInstance beam in m_LosaList)
            {
                ...
                t.Start("Create CreateTag2-NH");
                //Create tag on the beam's start and end.
                Reference beamRef = new Reference(beam);
                IndependentTag tag1 = IndependentTag.Create(m_revitDoc.Document, m_view.Id, beamRef, leader, tagMode, tagOrientation, curve.GetEndPoint(0));
                IndependentTag tag2 = IndependentTag.Create(m_revitDoc.Document, m_view.Id, beamRef, leader, tagMode, tagOrientation, curve.GetEndPoint(1));
