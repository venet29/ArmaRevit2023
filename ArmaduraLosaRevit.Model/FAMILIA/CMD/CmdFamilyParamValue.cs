...
namespace ArmaduraLosaRevit.Model.FAMILIA.CMD
{
    ...
    class CmdFamilyParamValue : IExternalCommand
    {
        ...
        /// </summary>
        void SetFamilyParameterValueFails(Document doc, string paramNameToAmend)
        {
            FamilyManager mgr = doc.FamilyManager;
            FamilyTypeSet familyTypes = mgr.Types;
            FamilyTypeSetIterator familyTypeItor = familyTypes.ForwardIterator();
        ...
        /// http://forums.autodesk.com/t5/revit-api/family-types-amp-shared-parameter-values/m-p/6218767
        /// </summary>
        void SetFamilyParameterValueWorks_todos_Tipo(Document doc, string paramNameToAmend, double valor)
        {
            FamilyManager mgr = doc.FamilyManager;
            FamilyParameter familyParam = mgr.get_Parameter(paramNameToAmend);
            ...
            if (familyParam != null)
            {
                ...
                {
                    if (familyType.Name == "") continue;
                    Debug.Print(familyType.Name);
                    mgr.CurrentType = familyType;
                    mgr.Set(familyParam, valor);
                }
            }
        }
        ...
        static string FamilyParamValueString(FamilyType t, FamilyParameter fp, Document doc)
        {
            ...
            switch (fp.StorageType)
            {
                ...
                case StorageType.ElementId:
                    ...
                    value = id.IntegerValue.ToString() + " (" + Util.ElementDescription(e) + ")";
                    break;
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            else
            {
                ...
                Debug.Print("\nFamily {0} has {1} parameter{2}.", doc.Title, n, Util.PluralSuffix(n));
          ...
          Debug.Print(
            ...
            values );
                ...
                keys.Sort();
                n = mgr.Types.Size;
                Debug.Print("Family {0} has {1} type{2}{3}", doc.Title, n, Util.PluralSuffix(n), Util.DotOrColon(n));
                foreach (FamilyType t in mgr.Types)
                {
                    ...
                    foreach (string key in keys)
                    {
                        ...
                        if (t.HasValue(fp))
                        {
                            string value = FamilyParamValueString(t, fp, doc);
            ...
            if (doc.IsFamilyDocument && exercise_this_method)
            {
                ...
                path = @"F:\\_revit\PROYECTO\\00.- LOSAS AUTOMATICO-29-01-2019\\EJERCICIO ZAMORA LOSA.rvt";
                if (0 < path.Length)
                {
                    app.Application.ExtractPartAtomFromFamilyFile(path, path + ".xml");
                }
            }
        ...
        public Result Metodo_Execute(Document docfm )
        {

            if (!docfm.IsFamilyDocument)
            {
               // message = "Please run this command in a family document.";
            }
            else
            {
                FamilyManager mgr = docfm.FamilyManager;

                int n = mgr.Parameters.Size;

                Debug.Print("\nFamily {0} has {1} parameter{2}.", docfm.Title, n, Util.PluralSuffix(n));
                ...
                keys.Sort();
                n = mgr.Types.Size;
                Debug.Print("Family {0} has {1} type{2}{3}", docfm.Title, n, Util.PluralSuffix(n), Util.DotOrColon(n));
                foreach (FamilyType t in mgr.Types)
                {
                    string name = t.Name;
                    Debug.Print("  {0}:", name);
                    foreach (string key in keys)
                    {
                        ...
                        if (t.HasValue(fp))
                        {
                            string value = FamilyParamValueString(t, fp, docfm);
                            Debug.Print("    {0} = {1}", key, value);
                        }
                    }
                }
            ...
            // family information as well:
            bool exercise_this_method = false;
            if (docfm.IsFamilyDocument && exercise_this_method)
            {
                string path = docfm.PathName;
                path = @"F:\\_revit\PROYECTO\\00.- LOSAS AUTOMATICO-29-01-2019\\EJERCICIO ZAMORA LOSA.rvt";
                if (0 < path.Length)
                {
                   // app.Application.ExtractPartAtomFromFamilyFile(path, path + ".xml");
        ...
        public Result Metodo_Execute_editParameter(Document docfm,string nombre , double valor)
        {
            ...
            else
            {
                ...
                foreach (FamilyParameter fp in mgr.Parameters)
                {
                    ...
                    {
                        SetFamilyParameterValueWorks_todos_Tipo(docfm, nombre, valor);
                    }
                }
