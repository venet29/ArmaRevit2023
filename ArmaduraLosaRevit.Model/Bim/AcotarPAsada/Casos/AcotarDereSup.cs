...
{
    public class AcotarDereSup : BaseAcotar, IAcotarCasos
    {

        public AcotarDereSup(UIApplication uiapp, EnvoltoriPasada envoltoriPasada, DimensionType _dimensionType) : base(uiapp, envoltoriPasada, _dimensionType)
        {
        ...
        public bool Ejecutar()
        {
            try
            {
                ...
                //importante
                //direccion
                bool result = dim1Planar.M2_ObtenerHaciaDere();

                if (!DibujarDimensionCentral(result)) return false;



                ListaPLanosPAsadas_aux = _envoltoriPasada.ListaPLanosPAsadas.OrderByDescending(c => c.Normal.X).ToList();
                ...
                if (dimension1 != null)
                {
                    if (_line_dimensionCentral.Length < LargoComprobarLargoMinimo_foot)
                        dimension1.TextPosition = dimension1.TextPosition + _envoltoriosPlanos.Direccion_ * DesplazamientoDimExtremo_foot + _envoltoriPasada.DireccionFAceNormalCAraDerecha* DesplazamientoDimCentral_foot;
                }
