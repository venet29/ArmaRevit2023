...
{
    public class BarraRoomGeometriaSX
    {
        // propeidades
        ...
        */
        public  Tuple<List<XYZ>, IList<Curve>> ListaFinal_ptov2(List<XYZ> listaPtos, UIApplication _uiapp,
                    ...
                    foreach (Face f in floor_.Faces)
                    {
                        ...
                        //if (Util.IsVertical(normal) && Util.PointsUpwards(normal))
                         ...
                         //   if (!IsTest) GraficarPtos(listaPtos[0], listaPtos[1], listaPtos[2], listaPtos[3]);
                            //ConstantesGenerales.sbLog.AppendLine("LIsta sin trasladar");
                            //if (listaPtos.Count == 4) ConstantesGenerales.sbLog.AppendLine(" XYZ p1 = new XYZ" + listaPtos[0].ToString() + ";");
                            //if (listaPtos.Count == 4) ConstantesGenerales.sbLog.AppendLine(" XYZ p2 = new XYZ" + listaPtos[1].ToString() + ";");
                            //if (listaPtos.Count == 4) ConstantesGenerales.sbLog.AppendLine(" XYZ p3 = new XYZ" + listaPtos[2].ToString() + ";");
                            //if (listaPtos.Count == 4) ConstantesGenerales.sbLog.AppendLine(" XYZ p4 = new XYZ" + listaPtos[3].ToString() + ";");
                           ...
                           // ConstantesGenerales.sbLog.AppendLine("Ptos trasladados");
                           // ConstantesGenerales.sbLog.AppendLine(p1.ToString());
                         //   ConstantesGenerales.sbLog.AppendLine(p2.ToString());
                           // ConstantesGenerales.sbLog.AppendLine(p3.ToString());
                           // ConstantesGenerales.sbLog.AppendLine(p4.ToString());
                          ...
                          //  Debug.Print("listaPtos[0] =" + listaPtos[0] + "P1=" + p1.ToString());
                           // Debug.Print("listaPtos[1] =" + listaPtos[1] + "P2=" + p2.ToString());
                          //  Debug.Print("listaPtos[2] =" + listaPtos[2] + "P3=" + p3.ToString());
                          //  Debug.Print("listaPtos[3] =" + listaPtos[3] + "P4=" + p4.ToString());
        ...
        //busca los vecinos para definir automaticamente que tipo de barra corresponde
        // caso diseño barra inferior automatico y caso Fauto manual
        /*
         resulta: se configuran estas tres variables:
         
           1) string tipoBarraPrincipal = ""      2) tipoBarra_izq_infer = "";  3)   tipoBarra_dere_sup = "";
             
             */
        private  string BuscarConfiParaCasoFauto(XYZ p1, XYZ p2, XYZ p3, XYZ p4, TipoOrientacionBarra TipoOrientacion)
        {
            //nota :
            //listBorde[0];//  izq  - infe
            //listBorde[1]; // dere - sup/
            string tipoBarraPrincipal = "";
            tipoBarra_izq_infer = "";
            tipoBarra_dere_sup = "";


            if (listBorde[0].obtenerRefereciasCercanas.roomNeighbour != null && listBorde[1].obtenerRefereciasCercanas.roomNeighbour != null)
            {

                //sin patas
                return tipoBarraPrincipal = "f3";
            }
            else if (listBorde[0].obtenerRefereciasCercanas.roomNeighbour == null && listBorde[1].obtenerRefereciasCercanas.roomNeighbour == null)
            {
                ...
                { return tipoBarra = "f3"; }  // si ambos espesores son mayores a 15cm, sin pata


            }
            else if (listBorde[0].obtenerRefereciasCercanas.roomNeighbour == null) // uno u otro
            {

                if (TipoOrientacion == TipoOrientacionBarra.Horizontal)
                {
                    if (p2.X < p3.X)
                        ubicacionBArraEnlosa = UbicacionLosa.Izquierda;
                    else
                        ubicacionBArraEnlosa = UbicacionLosa.Derecha;
                }
                else
                {
                    if (p2.X < p3.X)
                        ubicacionBArraEnlosa = UbicacionLosa.Inferior;
                    else
                        ubicacionBArraEnlosa = UbicacionLosa.Superior;
                }
            ...
            else if (listBorde[1].obtenerRefereciasCercanas.roomNeighbour == null) // uno u otro
            {//f1 0 f13
        ...
        private XYZ ObtenerPtoMouseParaDirectrizCasoAutomatico(ReferenciaRoomDatos refereciaRoomDatos, List<XYZ> listaPtosPerimetroBarras)
        {
            //lado izq - bajo
        ...
        private  Tuple<List<XYZ>, IList<Curve>> PtosParaBarraSuplev2(Transform Invertrans1, Transform InverTrans2_rotacion,
                                               XYZ p1, XYZ p2, XYZ p3, XYZ p4, double largoMinIzq, double largoMinDere)
        {
            ...
            if (p1.Y < p4.Y)  //p1 es menor hacia rriba
            {
                if (p1.X < p4.X)
                {
                    pf1 = new XYZ(p1.X - largoMinIzq * fact, p1.Y, p1.Z);
                    pf4 = new XYZ(p4.X + largoMinDere * fact, p1.Y, p4.Z);
                }
                else
                {
                    pf1 = new XYZ(p1.X + largoMinIzq * fact, p1.Y, p1.Z);
            ...
            else
            {
                ...
                {
                    // p1f   pf4
                    pf1 = new XYZ(p1.X - largoMinIzq * fact, p4.Y, p1.Z);
                    pf4 = new XYZ(p4.X + largoMinDere * fact, p4.Y, p4.Z);
                }
                else
                {
                    // p4f   p1f
            ...
            if (p2.Y > p3.Y)  //p1 es menor hacia rriba
            {
                if (p2.X < p3.X)
                {
                    pf2 = new XYZ(p2.X - largoMinIzq * fact, p2.Y, p2.Z);
                    pf3 = new XYZ(p3.X + largoMinDere * fact, p2.Y, p3.Z);
                }
                else
                {
                    pf2 = new XYZ(p2.X + largoMinIzq * fact, p2.Y, p2.Z);
            ...
            else
            {
                if (p2.X < p3.X)
                {
                    pf2 = new XYZ(p2.X - largoMinIzq * fact, p3.Y, p2.Z);
                    pf3 = new XYZ(p3.X + largoMinDere * fact, p3.Y, p3.Z);
                }
                else
                {
                    pf2 = new XYZ(p2.X + largoMinIzq * fact, p3.Y, p2.Z);
        ...
        */
        ...
        public  Tuple<List<XYZ>, double> ListaPoligonosRooms_S1_S3V2(UIApplication _uiapp,ReferenciaRoomDatos refRoomDatos, TipoOrientacionBarra orientacion)

        {
            ...
            List<XYZ> ListaInterseccionRoom2 =
                AnalizarRoom_S1_S3_1pto(_uiapp,refRoomDatos.PtoSeleccionMouse1, refRoomDatos.PtoSeleccionMouse2, orientacion, refRoomDatos.anguloBarraLosaGrado_1, refRoomDatos.Room2, refRoomDatos.Losa);
        ...
        private  List<XYZ> AnalizarRoom_S1_S3_1pto(UIApplication _uiapp,XYZ pto1, XYZ pto2, TipoOrientacionBarra orientacion, double Angle_pelotaLosa1_, Room m_roomSelecionado_1, Floor losaSeleccionada)
        {
            List<XYZ> ListaInterseccion = new List<XYZ>();
            ...
            if (boundaries.Count > 0)
            {
                ...
                {
                    LineAuxBuscarInters = Line.CreateBound(pto1, pto2);
                }
                ...
                for (int i = 0; i <= boundary_pts.Count - 2; i++)
                {
                    ...
                    //double angleu = Util.AnguloEntre2PtosGrados(direccionSeleccon, EndPoint - startPont);
                    //if (angleu > 90)
                    //{
