...
{
    public class f3_incli_esc : ARebarLosa, IRebarLosa
    {
       ...
       //private RebarInferiorDTO _rebarInferiorDTO;
        ...
        public f3_incli_esc(UIApplication uiapp, RebarInferiorDTO _rebarInferiorDTO, IGeometriaTag newGeometriaTag) : base(_rebarInferiorDTO)
        {
          ...
          //  this._RebarInferiorDTO = _rebarInferiorDTO;
           ...
           // ConstantesGenerales.sbLog.AppendLine($"deltaMOverAbajo : {Util.FootToCm(deltaMOverAbajo)}");
            ...
            _VectorMover = new XYZ(0, 0, ConstNH.DESPLAZAMIENTO_BAJO_Z_REBAR_FOOT + Util.CmToFoot(_rebarInferiorDTO.AcortamientoEspesorSecundario));
        ...
        {
            ConstNH.sbLog.AppendLine($"esepor losa : {_rebarInferiorDTO.espesorBarraEnLOsa_sinRecub_FooT}");
            ConstNH.sbLog.AppendLine($"ptoini  : {ptoini}");
            ConstNH.sbLog.AppendLine($"ptofin : {ptofin}");
            ConstNH.sbLog.AppendLine($"ptoini - ptofin : {(ptoini - ptofin).Normalize()}");
            ConstNH.sbLog.AppendLine($"ptofin -ptoini : {(ptofin - ptoini).Normalize()}");
            if (_vectorDireccionLosaInicialExternaInclinadaDTO.IsLosaEncontrada && _extHorDentroEscalera!=0) ladoAB = Line.CreateBound(ptoini + _vectorDireccionLosaInicialExternaInclinadaDTO.direccionLosa * _extHorDentroEscalera, ptoini);
            ladoBC = Line.CreateBound(ptoini, ptofin);
            if (_vectorDireccionLosaFinalExternaInclinadaDTO.IsLosaEncontrada && _extHorDentroEscalera != 0) ladoCD = Line.CreateBound(ptofin, ptofin + _vectorDireccionLosaFinalExternaInclinadaDTO.direccionLosa * _extHorDentroEscalera);
        }
