...
namespace ArmaduraLosaRevit.Model.ElementoBarraRoom.PathReinforment.Tipos
{
    ...
    class PathSymbolF17B : ChangeFamilyPathReinforceSymbol, IPathSymbol
    {
        ...
        public PathSymbolF17B(UIApplication uiapp, DatosNuevaBarraDTO _DatosNuevaBarraDTO, PathSymbol_REbarshape_FxxDTO _PathSymbolF20ADTO) : base(uiapp, _DatosNuevaBarraDTO)
        {
        ...
        public bool M1_ObtenerPArametros()
        {
            try
            {
                ...
                //a)
                double DesIzqInf_foot = 0;
                if (!Util.IsSimilarValor(largoDesfase_foot, pathSymbolFXXADTO.DesIzqInf_foot, 0.001))
                    DesIzqInf_foot = pathSymbolFXXADTO.DesIzqInf_foot + ConstNH.CONST_EXTRADESDFASE_FOOT;
                else
                    DesIzqInf_foot = pathSymbolFXXADTO.DesIzqInf_foot;

                PathReinforceSymbolDTO _pDS_DesIzqInf = new PathReinforceSymbolDTO("DesIzqInf", DesIzqInf_foot / _scaleView);
                ...
                //c) nombre familia con pata
                nombreFamiliaCOnPata = _datosNuevaBarraDTO.ObtenerNombrePAthSymbol_SoloEscala(_scala);
                nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_D" + Util.FootToCm(DesIzqInf_foot).ToString();
