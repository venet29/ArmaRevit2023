...
namespace ArmaduraLosaRevit.Model.ElementoBarraRoom.PathReinforment.Tipos
{
    ...
    class PathSymbolF22B : ChangeFamilyPathReinforceSymbol, IPathSymbol
    {
        private PathSymbol_REbarshape_FxxDTO pathSymbolFXXADTO;
        private List<ParametroNewTipoPathSymbol> ListaTipoFamilia_conpata;
#pragma warning disable CS0169 // The field 'PathSymbolF22B._PSD_eIzq' is never used
        private PathReinforceSymbolDTO _PSD_eIzq;
#pragma warning restore CS0169 // The field 'PathSymbolF22B._PSD_eIzq' is never used
#pragma warning disable CS0169 // The field 'PathSymbolF22B._PSD_eDere' is never used
        private PathReinforceSymbolDTO _PSD_eDere;
#pragma warning restore CS0169 // The field 'PathSymbolF22B._PSD_eDere' is never used
        private PathReinforceSymbolDTO _PSD_entre;

        public PathSymbolF22B(UIApplication uiapp, DatosNuevaBarraDTO _DatosNuevaBarraDTO, PathSymbol_REbarshape_FxxDTO _PathSymbolF20ADTO) : base(uiapp, _DatosNuevaBarraDTO)
        {
        ...
        public bool M1_ObtenerPArametros()
        {
            try
            {
                ...
                PathReinforceSymbolDTO _pDS_DesIzqInf = new PathReinforceSymbolDTO("DesIzqInf", (pathSymbolFXXADTO.DesIzqInf_foot > 0?pathSymbolFXXADTO.DesIzqInf_foot + ConstNH.CONST_EXTRADESDFASE_FOOT:0) / _scaleView);
                _listaPara_pathSymbol.Add(_pDS_DesIzqInf);

                PathReinforceSymbolDTO _pDS_DesDereSup = new PathReinforceSymbolDTO("DesIzqSup",( pathSymbolFXXADTO.DesIzqSup_foot >0?pathSymbolFXXADTO.DesIzqSup_foot + ConstNH.CONST_EXTRADESDFASE_FOOT:0) / _scaleView);
                ...
                if (Util.IsSimilarValor(pathSymbolFXXADTO.DesIzqInf_foot, 0, 0.001))
                    nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_D" + Util.FootToCm(pathSymbolFXXADTO.DesIzqSup_foot + ConstNH.CONST_EXTRADESDFASE_FOOT).ToString();
                else if (Util.IsSimilarValor(pathSymbolFXXADTO.DesIzqSup_foot, 0, 0.001))
                    nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_DI" + Util.FootToCm(pathSymbolFXXADTO.DesIzqInf_foot + ConstNH.CONST_EXTRADESDFASE_FOOT).ToString();
        ...
        private void CAlcularCAsoEstadar()
        {
            ...
            foreach (double item in listPAta)
            {
                ...
                //
                PathReinforceSymbolDTO _PDS_pataIzqInf = new PathReinforceSymbolDTO("DesIzqInf", Util.CmToFoot(item / _scaleView));
                _listaPara_pathSymbol_casoEstandar.Add(_PDS_pataIzqInf);
                PathReinforceSymbolDTO _pDS_DesIzqSup = new PathReinforceSymbolDTO("DesIzqSup", Util.CmToFoot(item / _scaleView));
                _listaPara_pathSymbol_casoEstandar.Add(_pDS_DesIzqSup);
