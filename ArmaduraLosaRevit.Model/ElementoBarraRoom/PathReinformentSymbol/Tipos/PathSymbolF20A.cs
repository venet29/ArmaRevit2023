...
namespace ArmaduraLosaRevit.Model.ElementoBarraRoom.PathReinforment.Tipos
{
    ...
    class PathSymbolF20A : ChangeFamilyPathReinforceSymbol, IPathSymbol
    {
        ...
        public PathSymbolF20A(UIApplication uiapp, DatosNuevaBarraDTO _DatosNuevaBarraDTO, PathSymbol_REbarshape_FxxDTO _PathSymbolF20ADTO) : base(uiapp, _DatosNuevaBarraDTO)
        {
        ...
        public bool M1_ObtenerPArametros()
        {
            try
            {
                ...
                double LargoDistintoCero_Foot = (Util.IsSimilarValor(pathSymbolFXXADTO.DesDereSup_foot, 0, 0.001)
                                                            ? pathSymbolFXXADTO.DesDereInf_foot
                                                            : pathSymbolFXXADTO.DesDereSup_foot);
                ...
                if (!Util.IsSimilarValor(largopata_foot, LargoDistintoCero_Foot))
                {
                    if (Util.IsSimilarValor(pathSymbolFXXADTO.DesDereInf_foot, 0, 0.001))
                        ...
                        nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_DI" + Util.FootToCm(pathSymbolFXXADTO.DesDereInf_foot).ToString();
                }
                //else if (Util.IsSimilarValor(pathSymbolFXXADTO.DesDereInf_foot, 0,0.001))
                //    nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_D" + Util.FootToCm(pathSymbolFXXADTO.DesDereSup_foot).ToString();
                else if(Util.IsSimilarValor(pathSymbolFXXADTO.DesDereSup_foot, 0,0.001)) 
                    nombreFamiliaCOnPata = nombreFamiliaCOnPata + "_DI" + Util.FootToCm(pathSymbolFXXADTO.DesDereInf_foot).ToString();
