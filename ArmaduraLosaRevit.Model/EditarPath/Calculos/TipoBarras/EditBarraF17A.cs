...
{
    public class EditBarraF17A : EditarPathRein_Base
    {
        public EditBarraF17A(UIApplication uiapp, SeleccionarPathReinfomentConPto _seleccionarPathReinfomentConPto) : base(uiapp, _seleccionarPathReinfomentConPto)
        {
        ...
        public void M1_3_MOdificarPArametrPrimary_Bar_Length_derecha(double _desplazamientoFoot)
        {
            ...
            if (_LargoPathRinforment.IsAlternative)
            {
                if (TipoCasoAlternativo_.TipoCasoAlternativo_ == TipoCasoAlternativo_enum.Definir)
                {
                    ...
                    {
                        Util.ErrorMsg("CAso F17A , desplazamineto derecho no implemnetado");
                    }
                    else
                    {
