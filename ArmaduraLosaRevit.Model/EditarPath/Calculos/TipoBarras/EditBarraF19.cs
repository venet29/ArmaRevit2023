...
{
    public class EditBarraF19 : EditarPathRein_Base
    {
        public EditBarraF19(UIApplication uiapp, SeleccionarPathReinfomentConPto _seleccionarPathReinfomentConPto) : base(uiapp, _seleccionarPathReinfomentConPto)
        {
        ...
        public void M1_3_MOdificarPArametrPrimary_Bar_Length_derecha(double _desplazamientoFoot)
        {
            ...
            if (_LargoPathRinforment.IsAlternative)
            {
                ...
                {
                    //Util.InfoMsg("La opcion 'Definir' se aplica para caso f19. Se utiliza caso proporcional");
                    //no aplica
                }
                else if (TipoCasoAlternativo_.TipoCasoAlternativo_ == TipoCasoAlternativo_enum.MantenerLargo)
                {
                    Util.InfoMsg("No implementado. Se utiliza caso proporcional");
