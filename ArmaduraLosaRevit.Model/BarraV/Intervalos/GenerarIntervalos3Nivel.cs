...
{
    public class GenerarIntervalos3Nivel : AGenerarIntervalosV, IGenerarIntervalosV
    {
        private IIniciarConXTraslapo _iniciarTraslapo;

        public GenerarIntervalos3Nivel(UIApplication _uiapp, ConfiguracionIniciaWPFlBarraVerticalDTO confiEnfierradoDTO, SelecionarPtoSup selecionarPtoSup, DatosMuroSeleccionadoDTO muroSeleccionadoDTO)
        ...
        public override void M1_ObtenerIntervaloBarrasDTO()
        {
            ...
            try
            {
                ...
                {
                    _iniciarTraslapo = new IniciarCon2Traslapo(_selecionarPtoSup, _confiWPFEnfierradoDTO);
                }
                else if (ComoPartirSegunLineaActual == 3)
                {
                    ListaIntervaloBarrasDTO.Clear();
                    Util.InfoMsg("Barra con traslapo de 3 nivel aun no implemnetado");
                    return;
                    //_iniciarTraslapo = new IniciarSinTraslapo(_selecionarPtoSup, _confiEnfierradoDTO);
                }
        ...
        private List<IntervaloBarrasDTO> M1_1_GEnerarListaIntervaloBarrasDTO()
        {
            ...
            foreach (CoordenadasBarra item in _iniciarTraslapo.ListaCoordenadasBarra)
            {
                ...
                XYZ PtoInicalAUX = _muroSeleccionadoInicialDTO.PtoInicioBaseBordeElemen_ProyectadoCaraElemenHost.AsignarZ(item.ptoBusqueda_muro.Z);
