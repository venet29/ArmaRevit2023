...
namespace ArmaduraLosaRevit.Model
{
    public class Creator
    {
        ...
        // these are
        // Autodesk.Revit.CreationDatosDiseño.IS_PATHREIN_AJUSTADO_LARGO
        // objects!
        ...
        public Creator(Document doc, UIApplication uiapp)
        {
        ...
        public Creator(Document doc_)
        {
            _doc = doc_;
            _credoc = doc_.Create;
            _creapp = doc_.Application.Create;
        }

        public Creator(UIApplication _uiapp)
        {
            uidoc = _uiapp.ActiveUIDocument;
            app = _uiapp.Application;

            _doc = _uiapp.ActiveUIDocument.Document;
            _credoc = _doc.Create;
            _creapp = _doc.Application.Create;
        }

        public Creator(ExternalCommandData commandData)
        {
        ...
        SketchPlane NewSketchPlanePassLine(Line line)
        {
        ...
        /// </summary>
        SketchPlane NewSketchPlaneContainCurve(Curve curve)
        {
        ...
        ModelCurve CreateModelCurve(Curve curve, XYZ origin, XYZ normal)
        {
            ...
            return _credoc.NewModelCurve(curve, sketchPlane);
        }

        public ModelCurveArray CreateModelCurves(Curve curve)
        {
            ...
            if (points.Count == 0)
            {
                Util.ErrorMsg("Error al obtener 'Tessellate'");
                return array;
            }
        ...
        #region dibujar poligonos --- modelLIne
        public ModelLine DrawPolygon(List<XYZ> loop)
        {
            XYZ p1 = XYZ.Zero;
            XYZ q = XYZ.Zero;
            bool first = true;
            ...
            return CreateModelLine(_doc, q, p1);
        }



        public List<ModelLine> DrawPolygon(List<XYZ> loop, BuiltInParameter builtInParameter)
        {
            List<ModelLine> listaModelLine = new List<ModelLine>();
            XYZ p1 = XYZ.Zero;
            XYZ q = XYZ.Zero;
            bool first = true;


            ModelLine modeline = null;
            foreach (XYZ p in loop)
            {
                if (first)
                {
                ...
                {
                    modeline = CreateModelLine(_doc, p, q);
                    if (modeline != null)
                    {
                        // modeline.get_Parameter(BuiltInParameter.OST_RoomSeparationLines);
                        listaModelLine.Add(modeline);
                    }
            ...
            modeline = CreateModelLine(_doc, q, p1);
            if (modeline != null)
                listaModelLine.Add(modeline);

            return listaModelLine;
        }

        // supone  List<XYZ> interno son dos ptos
        public List<ModelLine> DrawPolygon(List<List<XYZ>> loops, Level level)
        {
            List<ModelLine> listaModelLine = new List<ModelLine>();
            ModelLine modelLIne = null;
            foreach (List<XYZ> borde in loops)
            {
                if (level == null)
                { modelLIne = CreateModelLine(_doc, borde[0], borde[1]); }
                else
                { modelLIne = CreateModelLine(_doc, new XYZ(borde[0].X, borde[0].Y, level.ProjectElevation), new XYZ(borde[1].X, borde[1].Y, level.ProjectElevation)); }

                if (modelLIne != null)
                    listaModelLine.Add(modelLIne);
            }
            return listaModelLine;
        }

        public List<Line> DrawLIne(List<List<XYZ>> loops, Level level)
        {
            ...
            foreach (List<XYZ> borde in loops)
            {
                ...
                if (0.1 < borde[0].DistanceTo(borde[1]))
                {
                    if (level == null)
                    { LIne = Line.CreateBound(borde[0], borde[1]); }
                    else
                    { LIne = Line.CreateBound(new XYZ(borde[0].X, borde[0].Y, level.ProjectElevation), new XYZ(borde[1].X, borde[1].Y, level.ProjectElevation)); }
