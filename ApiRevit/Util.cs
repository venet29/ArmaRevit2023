...
namespace ApiRevit
{
    class Util
    {
        ...
        public static List<XYZ> Pick2Point(UIDocument uidoc, ObjectSnapTypes snapTypes)
        {
            List<XYZ> list = new List<XYZ>();
            snapTypes = ObjectSnapTypes.Endpoints | ObjectSnapTypes.Intersections;
            XYZ point1 = uidoc.Selection.PickPoint(snapTypes, "Select 1er punto");
            XYZ point2 = uidoc.Selection.PickPoint(snapTypes, "Select 2er punto");
            list.Add(point1);
            list.Add(point2);
    ...
    return e;
        ...
        /// <summary>nn
        /// 70)devuelve elemento preseleccionadonn
    ...
    public static class CompatibilityMethods
    {
          ...
          bool visibilidade)
        {
           ...
           .Name == "ExternalDefinitonCreationOptions").ToList();
            if (ls.Count > 0)
            {
                ...
                .GetMethod("Create", new Type[] { t }).Invoke(definitions, new object[] { ed }) as Definition;
            }
            else
            {
                value = definitions.GetType()
        ...
        public static char GetDecimalSymbol2(this Document doc)
        {
            ...
            {
                object temp = doc.GetType().GetProperty("ProjectUnit").GetValue(doc, null);
        ...
        {
            List<Type> ls = doc.GetType().Assembly
            .GetTypes().Where(a => a.IsClass && a.Name == "JoinGeometryUtils").ToList();
          ...
          List<Element> elementos)
        {
            ...
            MethodInfo met = obj.GetType().GetMethod("NewGroup", new Type[] { col.GetType() });
            if (met != null)
            {
          ...
          this Selection sel)
        {
            ...
            else
            {
                ...
                MethodInfo met = sel.GetType().GetMethod("SetElementIds", new Type[] { ids.GetType() });
        ...
        {
            ElementId value = null;
            PropertyInfo prop = view.GetType().GetProperty("ViewTemplateId");
            if (prop != null)
            {
                value = prop.GetValue(view, null) as ElementId;
            }
            return value;
        }
