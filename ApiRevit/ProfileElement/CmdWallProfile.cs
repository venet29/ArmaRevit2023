...
namespace ApiRevit.ProfileElement
{
    ...
    public class CmdWallProfile : IExternalCommand
    {
        ...
        static bool GetProfile(List<List<XYZ>> polygons, Solid solid, XYZ v, XYZ w)
        {
            double d, dmax = 0;
            ...
            foreach (Face f in faces)
            {
                ...
                if (null != pf
                  && Util.IsVertical(pf) // si es vertical
                  && Util.IsZero(v.DotProduct(pf.FaceNormal))) // si cara es de los costados   
                {
        ...
        /// </summary>
        static public List<List<XYZ>> GetWallProfilePolygons(          //Application app,
          List<Element> walls,
          Options opt)
        {
            XYZ p, q, v, w;
        ...
        public Result Execute1(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            List<List<XYZ>> polygons = GetWallProfilePolygons(walls, opt);
        ...
        void SetModelCurvesColor(ModelCurveArray modelCurves, View view, Color color)
        {
            foreach (var curve in modelCurves.Cast<ModelCurve>())
            {
        ...
        /// </summary>
        public Result Execute3(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIApplication uiapp = commandData.Application;
            UIDocument uidoc = uiapp.ActiveUIDocument;
            Application app = uiapp.Application;
            ...
            using (Transaction tx = new Transaction(doc))
            {
                ...
                Debug.Print("wall orientation {0}, face origin {1}, face normal {2}",
                ...
                Transform offset = Transform.CreateTranslation(voffset);
                ...
                foreach (var curveLoop in curveLoops)
                {
                    ...
                    if (wallCurve is Line)
                    {
                        ...
                        Plane plane = Plane.CreateByNormalAndOrigin(normal, forigin + voffset); // 2019
                        Debug.Print("plane origin {0}, plane normal {1}", Util.PointString(plane.Origin), Util.PointString(plane.Normal));
                        SketchPlane sketchPlane = SketchPlane.Create(doc, plane);
                        ModelCurveArray curveElements = credoc.NewModelCurveArray(curves, sketchPlane);
