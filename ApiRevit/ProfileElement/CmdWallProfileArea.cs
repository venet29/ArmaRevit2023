...
namespace ApiRevit.ProfileElement
{
   ...
   public class CmdWallProfileArea : IExternalCommand
    {
        ...
        */
        ...
        List<XYZ> ApplyTransform(List<XYZ> polygon, Transform t)
        {
            int n = polygon.Count;
            List<XYZ> polygonTransformed = new List<XYZ>(n);
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            List<List<XYZ>> polygons = CmdWallProfile.GetWallProfilePolygons(walls, opt);
            ...
            foreach (List<XYZ> polygon in polygons)
            {
                GetPolygonPlane(polygon, out normal, out d, out a);
                ...
                List<UV> polygon2d = CmdSlabBoundaryArea.Flatten(polygonHorizontal);

                double a2 = CmdSlabBoundaryArea.GetSignedPolygonArea(polygon2d);

                Debug.Assert(Util.IsEqual(a, a2), "expected same area from 2D and 3D calculations");
#endif
            ...
            for (i = 0; i < n; ++i)
            {
                Debug.Print("  Loop {0} area is {1} square feet{2}", i,
