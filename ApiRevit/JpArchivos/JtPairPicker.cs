...
namespace ApiRevit.JpArchivos
{
    ...
    /// picking.
    /// ///
    /// Elija un par de elementos de un tipo específico.
    /// Si existen exactamente dos en todo el modelo, tómalos. Si hay menos de dos, renunciar. Si los elementos han sido preseleccionados, use esos. De lo contrario, solicite un picking interactivo.
    /// </summary>
    /// 
    //https://thebuildingcoder.typepad.com/blog/2014/01/calculating-a-rolling-offset-between-two-pipes.html#2
    class JtPairPicker<T> where T : Element
    {
        ...
        public Result Pick()
        {
            ...
            _a = new List<T>(new FilteredElementCollector(_doc).OfClass(typeof(T)).ToElements().Cast<T>());
            int n = _a.Count;
            ...
            if (1 < n)
            {
                foreach (ElementId id in ids)
                {
                    ...
                    Debug.Assert(null != e, "only elements of type T can be picked");
                    _a.Add(e);
                    if (2 == _a.Count)
                    {
                        Debug.Print("Found two pre-selected " + "elements of desired type and " + "ignoring everything else.");
                        break;
                    }
                }
            }
            ...
            if (2 != _a.Count)
            {
                ...
                ISelectionFilter filter = new JtElementsOfClassSelectionFilter<T>();
                try
                {
                    Reference r = sel.PickObject(ObjectType.Element, filter, "Please pick first element.");
                    _a.Add(_doc.GetElement(r.ElementId) as T);
                }
                ...
                try
                {
                    Reference r = sel.PickObject(ObjectType.Element, filter, "Please pick second element.");
                    _a.Add(_doc.GetElement(r.ElementId) as T);
                }
                catch (Autodesk.Revit.Exceptions.OperationCanceledException)
                {
